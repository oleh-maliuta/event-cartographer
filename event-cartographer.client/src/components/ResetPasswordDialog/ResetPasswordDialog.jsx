    ...
    const resetPasswordPermissionRequest = useCallback(async (e) => {
        ...
        if (response.ok) {
            dispatchMessageState({
                type: 'SET_MESSAGES',
                payload: { mode: MessageStates.SUCCESS, list: [t('components.reset-password-dialog.success')] }
            });
        } else if (!response.ok) {
            if (json.message) {
                dispatchMessageState({
            ...
            <form className={`${cl.modal_window__form}`}
                ...
                <div className={`${cl.modal_window__content}`}>
                    <h1 className={`${cl.modal_window__header}`}>
                        {t('components.reset-password-dialog.header')}
                    </h1>
                    <p className={`${cl.modal_window__reset_password__description}`}>
                        {t('components.reset-password-dialog.description')}
                    </p>
                    <BlockMessage
                    ...
                    <PanelInput
                        ...
                        id='resetPassword-usernameOrEmail-input'
                        name='usernameOrEmail'
                        type='text'
                        autoComplete='username webauthn'
                        placeholder={t('components.reset-password-dialog.username-or-email-input')}
                        maxLength='500'
                        required
                        valueMissingValidity={t('components.reset-password-dialog.username_or_email_invalid.value_missing')} />
                </div>
                <div className={`${cl.modal_window__control}`}>
                    <div className={`${cl.modal_window__control__buttons}`}>
                        <button className={`${cl.modal_window__control__buttons__cancel}`}
                            ...
                            {t('components.reset-password-dialog.cancel')}
                        </button>
                        <button className={`${cl.modal_window__control__buttons__apply}`}
                            ...
                            disabled={sendingEmail}>
                            {
                                sendingEmail ?
                                    ...
                                    :
                                    <span>
                                        {t('components.reset-password-dialog.send-mail')}
                                    </span>
                            }
                        </button>
                    </div>
